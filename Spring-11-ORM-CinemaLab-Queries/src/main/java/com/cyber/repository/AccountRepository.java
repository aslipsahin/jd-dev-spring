...
public interface AccountRepository extends JpaRepository<Account, Long> {
    ...
    @Query("SELECT a.name FROM Account a WHERE a.role = ?1")
    List<String> findAllAdminAccounts(UserRole role);

    //Write a JPQL query to sort all accounts with age
    @Query("SELECT a FROM Account a ORDER BY a.age DESC")
    List<Account> findAllAccountsOrderByAge();
    ...
    List<Account> retrieveAllAgeLowerThank(Integer age);
