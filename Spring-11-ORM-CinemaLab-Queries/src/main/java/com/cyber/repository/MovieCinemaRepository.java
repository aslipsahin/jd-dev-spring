...
public interface MovieCinemaRepository extends JpaRepository<MovieCinema, Long> {
    ...
    //Write a derived query to find the top 3 expensive movies (join)
    List<MovieCinema> findFirst3ByOrderByMoviePriceAsc();

    //Write a derived query to list all movie cinemas that contain a specific movie name (join)
    List<MovieCinema> findAllByMovieNameContaining(String name);

    //Write a derived query to list all movie cinemas in a specific location (join)
    ...
    @Query(value = "SELECT count(*) FROM movie_cinema WHERE cinema_id = :cinema_id", nativeQuery = true)
    Integer countByCinemaIdNativeQuery(Long cinema_id);
